...
import {
  getVisibleFooterGroups,
  getFooterCTA,
} from "@/lib/siteSettingsService";
import type { SiteSettings } from "@/lib/types";
import { images } from '@/lib/images-supabase';

const FALLBACK_CTA_HEADING = "READY TO CREATE SOMETHING\nEXTRAORDINARY?";
const FALLBACK_CTA_SUBTEXT =
  "From intimate celebrations to large-scale productions, we bring creative direction, planning and execution together under one roof.";
              ...
              <div className="flex flex-col gap-3.5">
                {address && (
                  <ContactLine icon={<MapPin size={12} strokeWidth={1.5} />}>
                    <span className="font-sans text-[0.78rem] leading-[1.65] text-ivory/40">
                      {address}
                    </span>
                  </ContactLine>
                )}
          ...
          <div className="flex flex-col items-start gap-3 py-6 sm:flex-row sm:items-center sm:justify-between">
            <p className="font-sans text-[0.58rem] tracking-[0.1em] text-ivory/20">
              {settings?.copyright_text ||
                "© 2026 Fiesta Agency. All Rights Reserved."}
            </p>
            <div className="flex gap-5">
              <Link
                to="/privacy"
                className="font-sans text-[0.58rem] tracking-[0.1em] text-ivory/20 transition-colors duration-200 hover:text-gold/50"
              >
                Privacy Policy
              </Link>
              <Link
...
function SocialLink({
  ...
  return (
    <a
      ...
      aria-label={label}
      className="flex h-9 w-9 items-center justify-center rounded-full border border-white/10 bg-white/[0.02] text-ivory/45 transition-all duration-200 hover:-translate-y-0.5 hover:border-gold/40 hover:bg-gold/10 hover:text-gold"
    >
      {children}
    </a>
  );
...
function FooterCTA({
  ...
  return (
    <section
      ...
      style={{ height: "clamp(300px, 40vh, 400px)", minHeight: "280px" }}
    >
      ...
      <div className="relative z-10 flex h-full items-center justify-center px-6 md:px-12">
        <div className="flex max-w-[640px] flex-col items-center gap-5 text-center">
          <h2
            className="whitespace-pre-line font-serif font-light leading-none text-ivory"
          ...
          <p
            className="max-w-[440px] font-sans text-ivory/55 leading-[1.7]"
