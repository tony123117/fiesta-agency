...
import type { ServicesHeroContent } from '@/lib/types';

export function ServicesHeroEditor({ content, onChange }: { content: Record<string, unknown>; onChange: (c: Record<string, unknown>) => void }) {
  const data = content as unknown as ServicesHeroContent;
  const update = (fields: Partial<ServicesHeroContent>) => onChange({ ...data, ...fields });
