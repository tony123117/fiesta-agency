...
import type { HWWHeroContent } from '@/lib/types';

export function HWWHeroEditor({ content, onChange }: { content: Record<string, unknown>; onChange: (c: Record<string, unknown>) => void }) {
  const data = content as unknown as HWWHeroContent;
  const update = (fields: Partial<HWWHeroContent>) => onChange({ ...data, ...fields });
