...
import type { ServicesCTAContent } from '@/lib/types';

export function ServicesCTAEditor({ content, onChange }: { content: Record<string, unknown>; onChange: (c: Record<string, unknown>) => void }) {
  const data = content as unknown as ServicesCTAContent;
  const update = (fields: Partial<ServicesCTAContent>) => onChange({ ...data, ...fields });
