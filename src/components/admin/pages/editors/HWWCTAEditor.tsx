...
import type { HWWCTAContent } from '@/lib/types';

export function HWWCTAEditor({ content, onChange }: { content: Record<string, unknown>; onChange: (c: Record<string, unknown>) => void }) {
  const data = content as unknown as HWWCTAContent;
  const update = (fields: Partial<HWWCTAContent>) => onChange({ ...data, ...fields });
