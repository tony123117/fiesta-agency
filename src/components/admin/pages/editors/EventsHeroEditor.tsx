...
import type { EventsHeroContent } from '@/lib/types';

export function EventsHeroEditor({ content, onChange }: { content: Record<string, unknown>; onChange: (c: Record<string, unknown>) => void }) {
  const data = content as unknown as EventsHeroContent;
  const update = (fields: Partial<EventsHeroContent>) => onChange({ ...data, ...fields });
