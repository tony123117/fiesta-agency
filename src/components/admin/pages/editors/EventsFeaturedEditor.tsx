...
import type { EventsFeaturedContent } from '@/lib/types';

export function EventsFeaturedEditor({ content, onChange }: { content: Record<string, unknown>; onChange: (c: Record<string, unknown>) => void }) {
  const data = content as unknown as EventsFeaturedContent;
  const update = (fields: Partial<EventsFeaturedContent>) => onChange({ ...data, ...fields });
